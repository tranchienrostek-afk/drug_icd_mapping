...
- **Configuration Schema**: Không bao giờ dùng `None` hoặc giá trị hỗn hợp (union) trong cấu hình hệ thống quan trọng; Schema phải tường minh (explicit) để tránh runtime error.
- **Robustness over Speed**: Với Web Scraper, ưu tiên độ bền (fallback, relative XPath) hơn là cố gắng lấy dữ liệu nhanh bằng mọi giá bằng XPath tuyệt đối dễ gãy.
