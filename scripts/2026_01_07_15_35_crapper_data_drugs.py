    ...
    async with sem:
        ...
        try:
            ...
            record = {
                ...
                "noi_dung_dieu_tri": await clean_text(
                    page,
                    "#chi-dinh"
                ) or await clean_text(
                    page,
                    "xpath=//h2[contains(text(),'Chỉ định')]/following-sibling::div"
                ),
                "dang_bao_che": await clean_text(
                    page,
                    "xpath=//div[contains(text(),'Dạng bào chế')]/following-sibling::div"
                ),
                "danh_muc": await clean_text(
                    page,
                    "xpath=//div[contains(text(),'Nhóm thuốc')]/following-sibling::div"
                ),
                "ham_luong": await clean_text(
                    page,
                    "xpath=//div[contains(text(),'Hàm lượng')]/following-sibling::div"
                ),
                "url_nguon": link
            }
            return record
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # BLOCK RESOURCE (KHÔNG ĐỔI LOGIC)
        ...
        while current_page <= MAX_PAGES:
            ...
            for attempt in range(3):
                try:
                    ...
                    if await page.locator(
                        "xpath=/html/body/main/section[3]/div/div/div/div[2]/div/div/a"
                    ).count() > 0:
                        load_success = True
                        break
            ...
            if not load_success:
                log_error_page(current_page, "Không load được danh sách")
                current_page += 1
                continue
            ...
            try:
                ...
                print(f"   🔎 {len(links)} thuốc")

                tasks = [
                    scrape_detail(context, sem, link)
                    for link in links
                ]
                ...
                if batch_data:
                    ...
                    print(f"   💾 Đã lưu {len(batch_data)} thuốc")
                else:
                    log_error_page(current_page, "Trang trắng")

            except Exception as e:
                log_error_page(current_page, str(e))
